...
import { fetchVendorStats } from '@/lib/queries'
import { PageHeader, KpiCard, Section, Badge, HealthBar } from '@/components/ui'
import { formatRelativeTime, formatCents } from '@/lib/utils/quality'
...
async function fetchVendorProducts(vendorId: number) {
  const { data } = await sb
    .from('products')
    .select('id,title,category,brand,color,size,price_cents,sales_price_cents,availability,last_seen,image_url,variant_id,parent_product_url')
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Link href="/vendors" className="text-xs text-gray-500 hover:text-gray-700 flex items-center gap-1">
              <ArrowLeft className="w-3 h-3" /> Back
            </Link>
            <a
              href={vendor.url} target="_blank" rel="noreferrer"
      ...
      <div className="p-6 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          ...
          <KpiCard label="Health"      value={`${vendor.health_score}%`}
            tone={vendor.health_score >= 80 ? 'good' : vendor.health_score >= 60 ? 'warn' : 'danger'} />
          <KpiCard label="Miss. Category" value={vendor.missing_category} tone={vendor.missing_category > 0 ? 'warn' : 'default'} />
          <KpiCard label="Miss. Image"    value={vendor.missing_image_url} tone={vendor.missing_image_url > 0 ? 'warn' : 'default'} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Section title="Vendor info">
            <div className="space-y-3 text-sm">
              {[
                ...
                { label: 'Last seen',     value: vendor.latest_last_seen ? formatRelativeTime(vendor.latest_last_seen) : 'Never' },
          ...
          <Section title="Data health">
            <div className="space-y-3">
              {[
                { label: 'Has image_url', pct: Math.round(((vendor.total_products - vendor.missing_image_url) / Math.max(vendor.total_products, 1)) * 100) },
                { label: 'Has category',  pct: Math.round(((vendor.total_products - vendor.missing_category) / Math.max(vendor.total_products, 1)) * 100) },
                { label: 'Has color',     pct: Math.round(((vendor.total_products - vendor.missing_color) / Math.max(vendor.total_products, 1)) * 100) },
                { label: 'Has size',      pct: Math.round(((vendor.total_products - vendor.missing_size) / Math.max(vendor.total_products, 1)) * 100) },
                { label: 'Has variant',   pct: Math.round(((vendor.total_products - vendor.missing_variant_id) / Math.max(vendor.total_products, 1)) * 100) },
              ].map(({ label, pct }) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  {['', 'Title', 'Brand', 'Category', 'Color', 'Size', 'Price', 'Avail', 'Last seen'].map(h => (
                    <th key={h} className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-3 py-2.5">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {(products as any[]).map((p) => (
                  <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-3 py-2">
                      {p.image_url
                        ? <img src={p.image_url} alt="" className="w-8 h-8 rounded object-cover border border-gray-200" />
