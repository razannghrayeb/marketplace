    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12">
          {[
            { icon: Package, title: 'Products', desc: 'Manage your catalog', href: '/dashboard/products', color: 'bg-neutral-100 text-neutral-800' },
            { icon: TrendingUp, title: 'Analytics', desc: 'Catalog insights & performance', href: '/dashboard/analytics', color: 'bg-violet-100 text-violet-800' },
          ].map((item) => (
            <Link
              key={item.title}
              href={item.href}
              className="block p-6 rounded-2xl border border-neutral-200 bg-white hover:shadow-elevated hover:border-neutral-400 transition-all"
            >
              <div className={`w-12 h-12 rounded-xl ${item.color} flex items-center justify-center mb-4`}>
                <item.icon className="w-6 h-6" />
              </div>
              <h3 className="font-display font-semibold text-neutral-800">{item.title}</h3>
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 mb-8">
            <div className="p-4 rounded-xl bg-neutral-50">
              ...
              <p className="text-2xl font-bold text-neutral-800">{totalProducts}</p>
            </div>
            <div className="p-4 rounded-xl bg-neutral-50">
              <p className="text-sm text-neutral-500">Categories</p>
              <p className="text-2xl font-bold text-neutral-800">{categoryCount}</p>
            </div>
            <div className="p-4 rounded-xl bg-neutral-50">
              <p className="text-sm text-neutral-500">Brands</p>
              <p className="text-2xl font-bold text-neutral-800">{brandCount}</p>
            </div>
          </div>
