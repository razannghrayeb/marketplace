...
export default function CompleteStylePage() {
  ...
  if (isLoading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="h-8 w-48 bg-cream-200 rounded animate-pulse mb-8" />
  ...
  if (isError || !data) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        <p className="text-wine-600 mb-4">{(error as Error)?.message ?? 'Failed to load outfit suggestions'}</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      >
        <h1 className="font-display text-3xl font-bold text-charcoal-800 mb-2">Complete this look</h1>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          <div>
            <p className="text-sm font-medium text-charcoal-500 mb-2">Your item</p>
            ...
            <p className="mt-2 font-medium text-charcoal-800">{source.title}</p>
          </div>

          <div className="lg:col-span-3 space-y-10">
            {data.outfitSuggestion && (
              <div className="p-4 rounded-xl bg-gold-50 border border-gold-200">
                <p className="text-charcoal-700">{data.outfitSuggestion}</p>
              </div>
            )}
            ...
            {data.recommendations.map((rec, idx) => (
              <section key={idx}>
                <div className="flex items-center gap-2 mb-4">
                  <Shirt className="w-5 h-5 text-wine-600" />
                  <h2 className="font-display text-xl font-bold text-charcoal-800">{rec.category}</h2>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-cream-200 text-charcoal-600">
                    {rec.priorityLabel}
                  </span>
                </div>
                <p className="text-sm text-charcoal-500 mb-4">{rec.reason}</p>
