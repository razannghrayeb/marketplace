/**
 * Compare decision engine — request/response contract (frontend types align with backend).
 */
...
export type CompareMode = 'standard' | 'alter_ego'

export type CompareDecisionRequest = {
  productIds: number[]
...
export type DecisionConfidenceLevel = 'clear_choice' | 'leaning_choice' | 'toss_up'
...
export type PhotoRealityLabel = 'photo_stronger' | 'real_life_stronger' | 'aligned'

export type TensionAxisType =
  | 'safe_bold'
  | 'versatile_expressive'
  | 'polished_effortless'
  | 'practical_statement'
...
export type CompareDecisionResponse = {
  ...
  socialMirror?: {
    enabled: boolean
    explanation: Array<{ productId: number; message: string }>
  }
...
export const WINNER_CONTEXT_LABELS: Record<keyof CompareDecisionResponse['winnersByContext'], string> = {
  practical: 'Most practical',
  expressive: 'Most expressive',
  safest: 'Safest pick',
  mostExciting: 'Most exciting',
  currentSelf: 'Current-you fit',
  aspirationalSelf: 'Aspirational-you fit',
  value: 'Best value',
  quality: 'Best quality',
  style: 'Best style match',
  risk: 'Lowest risk',
  occasion: 'Occasion fit',
  overall: 'Overall lean',
}
