...
exports.normalizeArabic = exports.detectScript = exports.getTransliterationVariants = void 0;
exports.normalizeQuery = normalizeQuery;
exports.processQuery = processQuery;
exports.processQuerySync = processQuerySync;
exports.getQueryEmbedding = getQueryEmbedding;
const arabizi_1 = require("./arabizi");
Object.defineProperty(exports, "detectScript", { enumerable: true, get: function () { return arabizi_1.detectScript; } });
Object.defineProperty(exports, "normalizeArabic", { enumerable: true, get: function () { return arabizi_1.normalizeArabic; } });
Object.defineProperty(exports, "getTransliterationVariants", { enumerable: true, get: function () { return arabizi_1.getTransliterationVariants; } });
const spellCorrector_1 = require("./spellCorrector");
const dictionary_1 = require("./dictionary");
const llmRewriter_1 = require("./llmRewriter");
 ...
 */
function extractFilters(query, script) {
    const filters = {};
    let remaining = query;
    const dict = (0, dictionary_1.getDictionaries)();
    ...
    for (const [key, entry] of dict.attributes) {
        if (entry.normalizedTerm && remaining.toLowerCase().includes(entry.normalizedTerm)) {
            ...
            const attrData = (0, dictionary_1.findAttribute)(key);
            if (attrData) {
    ...
    for (const word of words) {
        const brand = (0, dictionary_1.findBrand)(word);
        if (brand) {
    ...
    for (const word of words) {
        const category = (0, dictionary_1.findCategory)(word);
        if (category) {
 ...
 */
function processArabizi(query) {
    const corrections = [];
    const words = query.split(/\s+/);
    for (const word of words) {
        const normalized = word.toLowerCase();
        // Check if it's a known Arabizi fashion term
        const fashionTerm = (0, arabizi_1.expandFashionTerm)(normalized);
        if (fashionTerm) {
            ...
            if (/[a-z]+[2-9]+[a-z]*/i.test(word) || /[2-9]+[a-z]+/i.test(word)) {
                ...
                const arabicForm = (0, arabizi_1.arabiziToArabic)(normalized);
 ...
 */
function expandBrandAliases(query) {
    const corrections = [];
    const words = query.split(/\s+/);
    const dict = (0, dictionary_1.getDictionaries)();
    for (const word of words) {
...
async function processQuery(query) {
    ...
    // Check cache first
    const cached = (0, cache_1.getCachedQuery)(query);
    if (cached) {
        return cached;
    }
    ...
    const script = (0, arabizi_1.detectScript)(normalizedQuery);
    ...
    const dict = (0, dictionary_1.getDictionaries)();
    const spellCorrections = (0, spellCorrector_1.correctQuery)(normalizedQuery, {
        brands: dict.brands,
    ...
    for (const correction of spellCorrections) {
        const alreadyExists = allCorrections.some(c => c.original.toLowerCase() === correction.original.toLowerCase());
        if (!alreadyExists) {
    ...
    if ((0, llmRewriter_1.shouldUseLLM)(normalizedQuery, script, allCorrections.length > 0, overallConfidence)) {
        const llmResult = await (0, llmRewriter_1.rewriteWithLLM)({
            originalQuery: query,
            normalizedQuery,
            script,
            allowedBrands: (0, dictionary_1.getAllBrandNames)(),
            allowedCategories: (0, dictionary_1.getAllCategoryNames)(),
            allowedGenders: (0, dictionary_1.getAllGenders)(),
        });
        if (llmResult && llmResult.confidence >= CONFIDENCE_THRESHOLDS.llmThreshold) {
            ...
            llmCorrection = {
                ...
                confidenceLevel: (0, spellCorrector_1.confidenceToLevel)(llmResult.confidence),
            };
            allCorrections.push(llmCorrection);
            // Update filters from LLM
            if (llmResult.extractedBrand)
                filters.brand = llmResult.extractedBrand;
            if (llmResult.extractedCategory)
                filters.category = llmResult.extractedCategory;
            if (llmResult.extractedGender)
                filters.gender = llmResult.extractedGender;
    ...
    const highConfidenceCorrections = allCorrections.filter(c => c.confidence >= CONFIDENCE_THRESHOLDS.autoApply);
    ...
    // Cache result
    (0, cache_1.cacheQuery)(query, result);
    return result;
}
/**
 * Process query synchronously (no LLM, for high-throughput)
 */
function processQuerySync(query) {
    const startTime = performance.now();
    // Check cache first
    const cached = (0, cache_1.getCachedQuery)(query);
    if (cached) {
...
async function getQueryEmbedding(query) {
    // Check cache
    const cached = (0, cache_1.getCachedEmbedding)(query);
    if (cached) {
        return cached;
    }
    if (!(0, image_1.isClipAvailable)()) {
        return null;
    }
    try {
        const embedding = await (0, image_1.getTextEmbedding)(query);
        (0, cache_1.cacheEmbedding)(query, embedding);
        return embedding;
    }
    catch (err) {
