...
exports.buildDictionaries = buildDictionaries;
...
exports.precomputeEmbeddings = precomputeEmbeddings;
...
exports.getAllBrandNames = getAllBrandNames;
...
async function loadBrandsFromDB() {
    try {
        ...
        const result = await core_1.pg.query(`SELECT DISTINCT brand, COUNT(*) as cnt 
       FROM products 
...
async function loadCategoriesFromDB() {
    try {
        const dictionary = getDictionaries();
        const result = await core_1.pg.query(`SELECT DISTINCT category, COUNT(*) as cnt 
       FROM products 
...
async function precomputeEmbeddings() {
    ...
    try {
        ...
        for (const [, entry] of dictionary.brands) {
            if (!entry.embedding) {
                try {
                    entry.embedding = await (0, image_1.getTextEmbedding)(entry.term);
                    computed++;
                }
                catch (err) {
...
function getAllGenders() {
    ...
    return Array.from(dict.attributes.values())
        .filter(e => e.type === "gender" || e.term === "men" || e.term === "women" || e.term === "kids" || e.term === "unisex")
        .map(e => e.term);
}
