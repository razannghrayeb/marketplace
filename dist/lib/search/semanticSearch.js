...
exports.loadBrandsFromDB = loadBrandsFromDB;
exports.loadCategoriesFromDB = loadCategoriesFromDB;
exports.calculateHybridScore = calculateHybridScore;
exports.countEntityMatches = countEntityMatches;
exports.buildSemanticOpenSearchQuery = buildSemanticOpenSearchQuery;
/**
 ...
 */
function classifyIntent(query, entities) {
...
async function loadBrandsFromDB() {
    try {
        const result = await core_1.pg.query(`SELECT DISTINCT LOWER(brand) as brand FROM products WHERE brand IS NOT NULL`);
...
async function loadCategoriesFromDB() {
    try {
        const result = await core_1.pg.query(`SELECT DISTINCT LOWER(category) as category FROM products WHERE category IS NOT NULL`);
        for (const row of result.rows) {
 ...
 */
function calculateHybridScore(semanticScore, keywordScore, entityMatches, weights = DEFAULT_WEIGHTS) {
 ...
 */
function countEntityMatches(product, entities) {
    let matches = 0;
 ...
 */
function buildSemanticOpenSearchQuery(parsedQuery, embedding, limit = 20) {
    ...
    if (entities.priceRange) {
        ...
        if (entities.priceRange.min)
            priceFilter.gte = entities.priceRange.min;
        if (entities.priceRange.max)
            priceFilter.lte = entities.priceRange.max;
