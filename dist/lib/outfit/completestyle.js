...
exports.CATEGORY_PAIRINGS = exports.COLOR_WHEEL = exports.CATEGORY_KEYWORDS = void 0;
exports.getComplementaryColors = getComplementaryColors;
exports.getAnalogousColors = getAnalogousColors;
exports.getColorHarmonies = getColorHarmonies;
exports.detectCategory = detectCategory;
exports.detectColor = detectColor;
exports.buildStyleProfile = buildStyleProfile;
exports.completeMyStyle = completeMyStyle;
exports.getProductForOutfit = getProductForOutfit;
exports.completeOutfitFromProductId = completeOutfitFromProductId;
...
function getComplementaryColors(color) {
    const colorInfo = exports.COLOR_WHEEL[color.toLowerCase()];
    ...
    return Object.entries(exports.COLOR_WHEEL)
...
function detectCategory(title, description) {
    ...
    const sortedCategories = Object.entries(exports.CATEGORY_KEYWORDS)
 ...
 */
function detectColor(title, description) {
    const text = `${title} ${description || ""}`.toLowerCase();
    // Use attribute extractor for color detection
    const attributes = (0, search_1.extractAttributesSync)(title);
    if (attributes.attributes.color) {
        return attributes.attributes.color;
    }
    // Fallback: check our color wheel
    for (const color of Object.keys(exports.COLOR_WHEEL)) {
 ...
 */
function buildStyleProfile(product) {
    ...
    const colorInfo = color ? exports.COLOR_WHEEL[color.toLowerCase()] : null;
    const colorProfile = {
...
function detectSeason(title, description) {
    const text = `${title} ${description || ""}`.toLowerCase();
    if (/winter|cold|warm|wool|fleece|puffer|down|thermal|cozy/i.test(text))
        return "winter";
    if (/summer|light|linen|cotton|breathable|cool|beach/i.test(text))
        return "summer";
    if (/spring|floral|pastel|light/i.test(text))
        return "spring";
    if (/fall|autumn|layering|knit/i.test(text))
        return "fall";
    return "all-season";
}
// ============================================================================
 ...
 */
async function completeMyStyle(product, options = {}) {
    const { maxPerCategory = 5, maxTotal = 20, priceRange, excludeBrands = [], preferSameBrand = false, useVisualSimilarity = true, disablePriceFilter = false, } = options;
    // Default price range: 0.5x to 2.5x of source product price
    // Only apply if product has a price and no explicit range provided
    const effectivePriceRange = priceRange ?? (!disablePriceFilter && product.price_cents > 0
        ? {
    ...
    // Get pairing rules
    const pairings = exports.CATEGORY_PAIRINGS[detectedCategory] || exports.CATEGORY_PAIRINGS.unknown;
    // Build recommendations for each pairing category
    const recommendations = [];
    let totalProducts = 0;
    for (const pairing of pairings) {
        if (totalProducts >= maxTotal)
            break;
        const categoryProducts = await findProductsForCategory(pairing.categories, detectedStyle, {
 ...
 */
async function findProductsForCategory(categories, style, options) {
    try {
        // Build category keywords
        const categoryKeywords = categories
            .flatMap(cat => exports.CATEGORY_KEYWORDS[cat] || [])
            .filter(k => k.length > 2);
        ...
        if (options.priceRange) {
            const priceFilter = { range: { price_cents: {} } };
            if (options.priceRange.min)
                priceFilter.range.price_cents.gte = options.priceRange.min;
            if (options.priceRange.max)
                priceFilter.range.price_cents.lte = options.priceRange.max;
        ...
        // Execute search
        const response = await core_1.osClient.search({
        ...
        for (const hit of hits) {
            ...
            if (product.color || detectColor(product.title)) {
                ...
                if (productColor) {
                    const isHarmonious = style.colorProfile.harmonies.some(h => h.colors.includes(productColor.toLowerCase()));
                    if (isHarmonious) {
                        matchScore += 15;
 ...
 */
function generateOutfitSuggestion(product, category, style, recommendations) {
 ...
 */
async function getProductForOutfit(productId) {
    try {
        const result = await core_1.pg.query(`
      SELECT 
 ...
 */
async function completeOutfitFromProductId(productId, options) {
