...
exports.getCategoryBaseline = getCategoryBaseline;
exports.computeCategoryBaseline = computeCategoryBaseline;
exports.computeAllCategoryBaselines = computeAllCategoryBaselines;
exports.analyzePriceAnomalies = analyzePriceAnomalies;
exports.getPriceLabel = getPriceLabel;
exports.getPriceReasons = getPriceReasons;
 ...
 */
function normalizeToUSD(priceCents, currency) {
 ...
 */
async function getCategoryBaseline(category) {
    const result = await core_1.pg.query(`SELECT * FROM category_price_baselines 
     WHERE category = $1 
     ORDER BY computed_at DESC 
     LIMIT 1`, [category.toLowerCase()]);
 ...
 */
async function computeCategoryBaseline(category) {
    // Get all active product prices in category
    const result = await core_1.pg.query(`SELECT 
       CASE 
     ...
     WHERE LOWER(category) = LOWER($1) 
       AND is_hidden = false 
       AND price_cents > 0
     ORDER BY price_usd`, [category, LBP_TO_USD]);
    const prices = result.rows.map((r) => r.price_usd);
    if (prices.length === 0) {
    ...
    await core_1.pg.query(`INSERT INTO category_price_baselines 
 ...
 */
async function computeAllCategoryBaselines() {
    const categories = await core_1.pg.query(`SELECT DISTINCT LOWER(category) as category 
     FROM products 
 ...
 */
async function getPriceHistoryForAnalysis(productId, days = 30) {
    const result = await core_1.pg.query(`SELECT price_cents, currency, sale_price_cents, recorded_at
     FROM price_history 
     WHERE product_id = $1 
       AND recorded_at > NOW() - INTERVAL '${days} days'
     ORDER BY recorded_at DESC`, [productId]);
    return result.rows;
}
/**
 * Analyze price volatility
 */
function analyzeVolatility(history, days = 14) {
    if (history.length < 2) {
 ...
 */
function analyzeDiscounts(history, currentSalePrice, currentPrice) {
 ...
 */
async function analyzePriceAnomalies(productId) {
    // Get current product data
    const productResult = await core_1.pg.query(`SELECT id, price_cents, currency, sales_price_cents, category
    ...
    // Analyze discounts
    const discountAnalysis = analyzeDiscounts(history, product.sales_price_cents, product.price_cents);
    // Get category baseline
    let baseline = null;
    let market_position = "unknown";
    let percentile;
    if (product.category) {
 ...
 */
function getPriceLabel(analysis) {
 ...
 */
function getPriceReasons(analysis) {
    const reasons = [];
    // Stability
