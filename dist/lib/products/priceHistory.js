...
exports.recordPricesBatch = recordPricesBatch;
exports.takePriceSnapshot = takePriceSnapshot;
exports.getPriceHistory = getPriceHistory;
exports.getPriceStats = getPriceStats;
exports.getPriceHistoryDaily = getPriceHistoryDaily;
exports.findPriceDrops = findPriceDrops;
/**
 ...
 */
async function recordPrice(productId, priceCents, currency = "LBP", salePriceCents, availability = true) {
    const result = await core_1.pg.query(`INSERT INTO price_history (product_id, price_cents, currency, sale_price_cents, availability)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`, [productId, priceCents, currency, salePriceCents, availability]);
...
async function takePriceSnapshot() {
    const products = await core_1.pg.query(`SELECT id, price_cents, currency, sales_price_cents, availability 
     FROM products 
...
async function getPriceHistory(productId, options = {}) {
    ...
    const result = await core_1.pg.query(`SELECT * FROM price_history 
...
async function getPriceStats(productId) {
    // Current price
    const currentResult = await core_1.pg.query(`SELECT price_cents FROM products WHERE id = $1`, [productId]);
    if (currentResult.rowCount === 0)
        return null;
    const currentPrice = currentResult.rows[0].price_cents;
    // Historical stats (last 90 days)
    const statsResult = await core_1.pg.query(`SELECT 
       MIN(price_cents) as lowest,
       MAX(price_cents) as highest,
       AVG(price_cents)::INTEGER as average
     FROM price_history 
     WHERE product_id = $1 AND recorded_at > NOW() - INTERVAL '90 days'`, [productId]);
    // Price 30 days ago
    const price30dResult = await core_1.pg.query(`SELECT price_cents FROM price_history 
 ...
 */
async function getPriceHistoryDaily(productId, days = 30) {
 ...
 */
async function findPriceDrops(thresholdPercent = 10, sinceDays = 1) {
    const result = await core_1.pg.query(`WITH recent_prices AS (
     ...
     WHERE o.old_price > r.new_price
       AND ((o.old_price - r.new_price)::FLOAT / o.old_price) * 100 >= $1`, [thresholdPercent]);
    return result.rows;
}
