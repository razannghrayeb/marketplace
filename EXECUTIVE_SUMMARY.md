...
**Production hardening (verify before launch):**
1. Protect **`/admin`** with auth + admin role (currently a known gap).
2. Set strong **`JWT_SECRET`** and database/Vertex/R2 envs in production.
3. Confirm **`cart_items`** and try-on migrations exist on the target database.
4. Resolve **`006_*` migration ordering** if you use a strict sequential runner.

**Missing product flows (non-blocking):**
- Call **`POST /api/auth/logout`** with refresh token to blacklist it (access token still short-lived)
- No checkout or orders (cart only)
- No email verification or password reset

**Try-on:** Backend is complete; the Next.js app includes a try-on flow when **`GCLOUD_PROJECT`**, **`007_virtual_tryon.sql`**, and **`x-user-id`** are wired. See **`docs/FEATURES.md`**.
...
## 🚨 Critical Issues (verify)

| Issue | Action |
|--------|--------|
| **Admin routes** | Add `requireAuth` + `requireAdmin` on `/admin` before public deploy. |
| **JWT secret** | Never ship with default `JWT_SECRET`. |
| **Migrations** | Ensure `cart_items`, try-on, and wardrobe tables exist on prod DB; fix `006_*` ordering if needed. |

OpenSearch **`ensureIndex()`** runs when `NODE_ENV !== "test"` — confirm `OS_*` env vars on deploy.

**Feature ↔ API map:** **`docs/FEATURES.md`**
...
10. ~~Implement logout + token blacklist~~ — done (`POST /api/auth/logout`)
...
| Dimension | Score | Why |
|-----------|-------|-----|
| **Feature Completeness** | A+ (97%) | Core features in; checkout/orders still missing |
...
**Generated by:** Claude Code Senior Engineering Audit — March 17, 2026 (footer refreshed March 2026 for doc accuracy)
