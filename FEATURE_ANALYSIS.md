...
1. **Limited Query Understanding:**
   - No advanced NLP for complex queries ("show me dresses under $100 similar to Zara style but in blue")
   - No conversational context (multi-turn queries)
   - Limited understanding of negation ("not too formal")
...
- Add spell correction using Levenshtein/ML-based approach
- Implement query autocomplete with trending searches

**Medium Priority:**
- Add conversational search with context tracking
...
3. **Missing Features:**
   - No reverse image search by URL (only file upload)
...
- Implement image search by URL (not just upload)
...
3. **Prompt Engineering Required:**
   ...
   - Users need to learn prompt patterns
...
**High Priority:**
- Add prompt templates/suggestions for users
- Implement visual attribute preview (show detected color/style)
