...
**Date:** March 2026 (feature paths refreshed; see **`docs/FEATURES.md`** for the canonical endpoint map)
...
**Location:** Primary enhanced path: `src/routes/search/search.service.ts` (`textSearch`), `fashionSearchFacade.searchText`; legacy/helpers in `src/lib/search/semanticSearch.ts`.

**Public endpoints:** **`GET /search?q=`** (mounted at `/search`, not `/api/search`). Catalog browse search: **`GET /products/search`**.
...
### Implementation Overview
**Location:** `src/routes/products/products.controller.ts` (`searchProductsByImage`), `src/lib/search/fashionSearchFacade.ts` (`searchImage`), `src/routes/products/products.service.ts` (`searchByImageWithSimilarity`). Alternate mount: `POST /search/image` in `search.controller.ts` (same facade).

**Preferred storefront path:** **`POST /products/search/image`** (multipart `image`).
...
**Pipeline (simplified):**
```
Upload → CLIP (+ optional garment vec, pHash, capped BLIP, quick color hints)
  → OpenSearch kNN (embedding) + attribute cosines + category rerank
  → computeHitRelevance → Results
```
See **`docs/embeddings-and-search-pipelines.md`** §4.
...
**Architecture:**
- **Google Cloud Vertex AI** Virtual Try-On API (default model id `virtual-try-on-001` in `config.ts`; override with `TRYON_MODEL`)
...
| `TRYON_MODEL` | `virtual-try-on-001` (default) | No |
...
**Generated March 2026 — periodically reconciled with `docs/FEATURES.md` and `docs/IMPLEMENTATION_STATUS.md`**
